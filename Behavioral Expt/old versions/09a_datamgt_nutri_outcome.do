...
merge 1:m uniqueID using "D:\GoogleDrive\jy_mrt_files\MRT - DFC (2017-2018)\Data analysis\DFC - data\merged files\collapsed data\merge_collapseALL.dta"
sort uniqueID
drop _merge
...
keep uniqueID session hh round carb_round pro_round fat_round energy_round ///
	 ...
	 D2_h D2_w D3_h D3_w D4_h D4_w D5_h D5_w I1_h  I1_w ///          /*(3) INDIVIDUAL – Questionnaire for ENUMERATOR*/
	 M1_h M1_w M3_h M3_w S1_h S1_w ///
	 District s06_urba s06_rura s07 s3q1 s3q2a s3q2b s3q2c s3q2d s3q2e s3q2f s3q3d /// /*2018 Consumer Survey*/
	 s3q4_1 s3q4_2 s3q4_3 s3q4_4 s3q4_5 s3q4_6_o /// 
	 s3q5_01 s3q5_02 s3q5_03 s3q5_04 s3q5_05 s3q5_06 ///
	 s4q1_06 s4q1_07 s6q3 s6q6 q1a_06 s6q18_to
...
label variable carb_round "amount of Carbohydrates (in kcal) consumed per round"
label variable pro_round "amount of Protein (in kcal) consumed per round"
label variable fat_round "amount of fat (in kcal) consumed per round"
label variable energy_round "total amount of energy (in kcal) consumed per round"
...
***********************DEPENDENT VARIABLES********************************
gen carb_percap=carb_round/hhsize
gen pro_percap=pro_round/hhsize
gen fat_percap=fat_round/hhsize
gen energy_percap=energy_round/hhsize

summarize carb_percap pro_percap fat_percap energy_percap

gen sharecarb_percap=carb_percap/energy_percap
gen sharepro_percap=pro_percap/energy_percap
gen sharefat_percap=fat_percap/energy_percap
gen tot_prop=sharecarb_percap+sharepro_percap+sharefat_percap

summarize sharecarb_percap sharepro_percap sharefat_percap tot_prop
drop tot_prop
...
gen actual_2daybudget= ((actual_monbudget/4.3)/7)*2 /*from 2018 consumer survey--"/4.3)/7*2" factor was added to adjust budget for two days*/
...
save "D:\GoogleDrive\jy_mrt_files\MRT - DFC (2017-2018)\Data analysis\DFC - data\merged files\09analysis_regn_kcal.dta", replace
...
label variable carb_percap "amount of Carbohydrates (in kcal) consumed per capita (by round)"
label variable pro_percap "amount of Protein (in kcal) consumed per capita (by round)"
label variable fat_percap "amount of Fat (in kcal) consumed per capita (by round)"
label variable energy_percap "amount of total Energy (in kcal) consumed per capita (by round)"

label variable sharecarb_percap "share of carbohydrates on all dishes  consumed in a household (by round)"
label variable sharepro_percap "share of protein on all dishes  consumed in a household (by round)"
label variable sharefat_percap "share of fat on all dishes  consumed in a household (by round)"
...
suumarize wchild wteens wadult wseniors
